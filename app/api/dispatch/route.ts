...
export async function POST(req: NextRequest) {
  try {
    ...
    const hospitalWithTRS = computeTRS(hospital, incident.lat, incident.lng, incident.severity_tier as any, bloodGroup)
    ...
    if (familyResults.length > 0) {
      await supabase.from('notifications').insert(
        contacts.map((c, i) => ({
          incident_id, recipient: c.phone, channel: 'sms+whatsapp',
    ...
    return NextResponse.json({
      success: true, hospital: hospitalWithTRS, eta_min,
      family_notified: familyResults.length, hospital_alerted: hospitalAlert.success,
    })
  } catch (e) {
